<?php
/**
 * Mezza Card payment method
 */
class WC_Payment_Gateway_Cowpay_Meeza_Card extends WC_Payment_Gateway_Cowpay
{
    ...
    function __construct()
    {
        ...
        $this->id = "cowpay_meeza_card";

        // The Title shown on the top of the Payment Gateways Page next to all the other Payment Gateways
        $this->method_title = esc_html__("Cowpay Meeza Card", 'woo-cowpay');

        // The description for this Payment Gateway, shown on the actual Payment options page on the backend
        $this->method_description = esc_html__("Cowpay Pay Using Meeza Card Payment Gateway for WooCommerce", 'woo-cowpay');

        // The title to be used for the vertical tabs that can be ordered top to bottom
        $this->title = esc_html__("Cowpay Meeza Card", 'woo-cowpay');

        // If you want to show an image next to the gateway's name on the frontend, enter a URL to an image.
        //$this->icon = plugin_dir_url(__FILE__) . 'LOGO.png';
        $this->icon = WOO_COWPAY_PLUGIN_URL . '/public/images/meeza_wallet_logo.png';
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            ...
            'title' => array(
                ...
                'default'    => __('Meeza Card', 'woo-cowpay'),
            ),
            'description' => array(
                ...
                'default'    => __('Pay using Meeza Card', 'woo-cowpay'),
                'css'        => 'max-width:350px;'
            ),
        );
    }
    ...
    {
        woo_cowpay_view("meeza-card-payment-fields"); // have no data right now
    }
